...
import java.com.lechuang.module.bean.MyCardUsedRecordBean;
 ...
 * @describe:
 */
@Route(path = ARouters.PATH_MY_CARD_USED)
public class MyCardUsedFragment extends LazyBaseFragment implements OnRefreshLoadMoreListener {
...
//        int i = fenSiActivity;
    ...
    public void getAllData() {
        ...
        NetWork.getInstance ()
                .setTag ( Qurl.myCardUsed )
                .getApiService ( ModuleApi.class )
                .myCardUsedRecord ( allParam )
                .subscribeOn ( Schedulers.io () )
                .observeOn ( AndroidSchedulers.mainThread () )
                .subscribe ( new RxObserver<MyCardUsedRecordBean> ( context==null?getActivity ():context
                        , false, false ) {

                    @Override
                    public void onSuccess(MyCardUsedRecordBean result) {
                        if (result == null) {
    ...
    private List<MyCardUsedRecordBean.ListBean> mProductList;
    private BaseQuickAdapter<MyCardUsedRecordBean.ListBean, BaseViewHolder> mBaseProductQuickAdapter;
    private void setFansData(List<MyCardUsedRecordBean.ListBean> fansList) {
        ...
        if (mBaseProductQuickAdapter == null) {
            mBaseProductQuickAdapter = new BaseQuickAdapter<MyCardUsedRecordBean.ListBean, BaseViewHolder>(R.layout.item_mycardrecord_list, mProductList) {
                @Override
                protected void convert(BaseViewHolder helper, MyCardUsedRecordBean.ListBean item) {
                    try {
                        //使用专区 tv_name 什么卡 tv_card 时间 tv_time
                        helper.setText(R.id.tv_name, item.remark);
                        helper.setText(R.id.tv_card, item.useMiCardDetail);
                        helper.setText(R.id.tv_time, item.useTime);
                        ...
                        if (item.agencyLevel==1){
                            ivFansVip.setBackground ( getResources ().getDrawable ( R.drawable.img_fans_vip ) );
                        }else{
