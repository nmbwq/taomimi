...
@Route(path = ARouters.PATH_OTHER_PRODUCT)
public class OtherProductActivity extends BaseActivity implements OnRefreshLoadMoreListener, View.OnClickListener {

    @Autowired(name = Constant.TITLE)
    public String title = "";//界面传递的值标题，
    @Autowired(name = Constant.CLASSTYPEID)
    ...
    protected int getLayoutId() {
        return R.layout.activity_product_jihe;
    }
...
//        mNesProduct.setTopTabLayout(mShaiXuanProBottom,mShaiXuanProTop);
    ...
    protected void initView() {
        ...
        mDeviceId=getDeviceId(OtherProductActivity.this);
    ...
    private void getProductList(boolean isShowProgress) {
        ...
        if (!TextUtils.isEmpty(keyword)){
            allParam.put("name", keyword);
        }

        allParam.put("classTypeId", classTypeId);
    ...
    private void setBannerData(List<SearchResultBean.IndexBannerListBean> indexBannerList) {
    ...
    private void setAdapter() {
        if (mBaseProductQuickAdapter == null) {
            mBaseProductQuickAdapter = new BaseQuickAdapter<SearchResultBean.ProductListBean, BaseViewHolder>(this.mIsSingleLine ? R.layout.item_product_product1 : R.layout.item_product_product2, mProductList) {
                @Override
...
//                    routerBean.i = mProductList.get(position).tbItemId;
                    ...
                    LinkRouterUtils.getInstance().setRouterBean(OtherProductActivity.this, routerBean);
                }
            });
