...
public class MineCoverFlowViewPager extends RelativeLayout implements OnPageSelectListener {


    /**
     * 适配器
     */
    private MineCoverFlowAdapter mAdapter;
    ...
    public MineCoverFlowViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void init() {
        // 构造适配器，传入数据源
        mAdapter = new MineCoverFlowAdapter(mViewList,getContext());
        // 设置选中的回调
    ...
    public void setViewList(List<String> imageUrls, @LayoutRes int layoutItem){
        ...
        for(int i = 0; i < imageUrls.size() ;i++){
            ...
            inflate.setAlpha(1f);
//            inflate.setTranslationX(mAdapter.dp2px(-20));
            final ImageView vpItemImg = inflate.findViewById(R.id.vp_item_img);
            final int finalI = i;
            Glide.with(BaseApplication.getApplication()).load(imageUrls.get(i)).asBitmap().into(new SimpleTarget<Bitmap>() {
                @Override
    ...
    public void setOnPageClickListener(OnPageClickListener onPageClickListener){
        ...
        mAdapter.setOnPageClickListener(mPageClickListener);
    }
